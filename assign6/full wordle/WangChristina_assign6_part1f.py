"""
Christina Wang	8/6/22	CSCI-UA 2 - 002
Assignment #6 Problem #1f
"""
...
#open file for appending
    wordlestatistics = open('wordlestatistics.txt', 'a')
...
#write score to the file
    wordlestatistics.write(str(score))

#write a line break
    wordlestatistics.write('\n')

#close the file
    wordlestatistics.close()
...
##play again
    ...
    if play_again == "n" or play_again =="N":
        print()
#open file for reading
        wordlestatistics = open('wordlestatistics.txt', 'r')
#read in all data as one long string
        allscore = wordlestatistics.read()
#cut apart string based on \n"
        splitscore= allscore.split("\n")
#overall high score
        splitscore.sort()
        highscorelist=list(filter(("").__ne__, splitscore))
        highscorelist1=list(filter(("0").__ne__, highscorelist))
        print("High score: ", *highscorelist1[:1])
#average number of tries
        highscorelistint= list(map(int, highscorelist1))
        avg=sum(highscorelistint)/(len(highscorelistint))
        print("Average Score: ", format(avg, '.2f'))
#number of tries when they lost
        lost= "0"
        num_lost = splitscore.count(lost)
        print("Number of lost games: ", num_lost)
#close file
        wordlestatistics.close()
        break
